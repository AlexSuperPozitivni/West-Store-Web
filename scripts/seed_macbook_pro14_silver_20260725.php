<?php
// One-off (2026-07-25): set MacBook Pro 14" M5 Pro — Silver / 15 CPU 16 GPU / 24GB / 1TB (MGDN4) price = 210000.
// Recomputes the product "from" price/price_max from non-zero variation prices.
// Run on prod: php artisan tinker scripts/seed_macbook_pro14_silver_20260725.php
...
foreach ($vars as $i => $v) {
    $a = $v['attributes'] ?? [];
    if (($a['Цвет'] ?? '') === 'Серебристый'
        && ($a['SSD'] ?? '') === '1 ТБ'
        && ($a['Процессор'] ?? '') === '15 CPU / 16 GPU') {
        $vars[$i]['price'] = 210000;
        $vars[$i]['regular_price'] = 210000;
        $changed++;
    }
}
$p->variations = $vars;
